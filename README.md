...
and renders it **natively in the terminal** as styled ANSI text (think `glow`/`bat`,
...
```sh
brew tap dsaad68/termdown
brew install dsaad68/termdown/termdown
```

> **Note:** while the repository is private, the tap and its release binaries
> require authentication — set a `HOMEBREW_GITHUB_API_TOKEN` (a GitHub token with
> `repo` scope) before tapping. Once the repository is made public, the commands
> above work without any token.

To upgrade later: `brew upgrade termdown`.
...
|---|---|---|---|
| `theme` | string | see list below | Content color palette (unknown → `dark`) |
...
- **Mouse** is off by default. Enable it with `--mouse` on the CLI or
  ...
  cursor (pager) or selects a file — clicking the highlighted file opens it
